...
/**
 * Script to seed all 1-5 boxes Canada order config files sequentially
 *
 * Usage:
 *   npm run seed-1to5-boxes-canada-orders
 *   npm run seed-1to5-boxes-canada-orders -- --skip-confirmation
 *   npm run seed-1to5-boxes-canada-orders -- --use-direct-mode
 *   npm run seed-1to5-boxes-canada-orders -- --start-from 5
 */
...
const CONFIG_PREFIX = "canada-orders-1to5-boxes-batch-";
...
function parseArgs(): Options {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--skip-confirmation") {
      ...
      console.log(`
Usage: npm run seed-1to5-boxes-canada-orders [options]
...
Examples:
  npm run seed-1to5-boxes-canada-orders
  npm run seed-1to5-boxes-canada-orders -- --skip-confirmation
  npm run seed-1to5-boxes-canada-orders -- --use-direct-mode --continue-on-error
  npm run seed-1to5-boxes-canada-orders -- --start-from 5
  npm run seed-1to5-boxes-canada-orders -- --max-batches 5
`);
      process.exit(0);
    }
  }
...
/**
 * Find all 1-5 boxes order config files and return them sorted by batch number
 */
function find1to5BoxesConfigs(): Array<{ batchNumber: number; path: string }> {
  ...
  for (const file of files) {
    if (file.startsWith(CONFIG_PREFIX) && file.endsWith(".json")) {
      // Extract batch number from filename: canada-orders-1to5-boxes-batch-01.json -> 1
      const match = file.match(/canada-orders-1to5-boxes-batch-(\d+)\.json/);
      if (match) {
...
function seedConfig(batchNumber: number, configPath: string, totalConfigs: number, options: Options): boolean {
  ...
  console.log(`\n${"=".repeat(60)}`);
  console.log(`📦 Seeding batch ${paddedNumber}/${totalConfigs}: canada-orders-1to5-boxes-batch-${paddedNumber}.json`);
...
async function main(): Promise<void> {
  ...
  // Find all 1-5 boxes order config files
  const allConfigs = find1to5BoxesConfigs();

  if (allConfigs.length === 0) {
    console.error("❌ No 1-5 boxes order config files found!");
  ...
  console.log("\n🚀 Starting batch seeding for 1-5 boxes Canada order configs");
