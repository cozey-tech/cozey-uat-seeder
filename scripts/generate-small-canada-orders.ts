...
/**
 * Generate small, reasonable Canada order configs
 * Focuses on 1-item orders (single sofas, cover kits) and small 2-3 item orders
 * Perfect for testing without huge complex orders
 *
 * Usage:
 *   npx tsx scripts/generate-small-canada-orders.ts [--seed <number>] [--batches <number>]
 *
 * Options:
 ...
 *
 * Examples:
 *   npx tsx scripts/generate-small-canada-orders.ts
 *   npx tsx scripts/generate-small-canada-orders.ts --seed 12345 --batches 15
 */
...
/**
 * Generate a small order (1-3 items, quantity 1-2)
 */
function generateSmallOrder(
  ...
  if (orderType === "regular-only") {
    ...
    if (itemCount === 1) {
      ...
      // Three items - 2 regular, 1 pnp (or vice versa)
      const regularCount = rng.random() < 0.5 ? 2 : 1;
      const pnpCount = 3 - regularCount;
...
/**
 * Generate a single config file with small orders
 */
function generateConfig(catalog: SKUCatalog, customers: Customer[], batchNumber: number, rng: SeededRNG): SeedConfig {
  ...
  // Distribution for small orders:
  // - 60% single-item orders (12 orders)
  // - 30% two-item orders (6 orders)
  // - 10% three-item orders (2 orders)
  // - Order types: 40% regular-only, 40% pnp-only, 20% mixed

  const singleItemCount = 12;
  const twoItemCount = 6;
  const threeItemCount = 2;
...
function validateConfig(
  ...
  // Check that orders are small (1-3 items)
  const itemCounts = { 1: 0, 2: 0, 3: 0, other: 0 };
  ...
  if (itemCounts.other > 0) {
    warnings.push(`Found ${itemCounts.other} orders with more than 3 items (expected only 1-3 item orders)`);
  }
...
async function main(): Promise<void> {
  ...
  console.log("🚀 Starting small Canada order config generation...");
  console.log(`   Focus: Small orders (1-3 items, quantity 1-2)`);
  ...
  for (const { batchNumber, config } of configs) {
    const filename = `canada-small-orders-batch-${batchNumber.toString().padStart(2, "0")}.json`;
  ...
  console.log(`✅ All orders are small (1-3 items, quantity 1-2)`);
  if (args.seed !== undefined) {
